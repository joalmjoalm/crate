...
public class UnionPhase extends AbstractProjectionsPhase implements UpstreamPhase {
    ...
    public UnionPhase(UUID jobId,
        ...
        if (projections.isEmpty()) {
            outputTypes = Iterables.getLast(mergePhases).outputTypes();
        } else {
    ...
    public void readFrom(StreamInput in) throws IOException {
        ...
        int numPhases = in.readVInt();
        Collection<MergePhase> phases = new ArrayList<>(numPhases);
        for (int i = 0; i < numPhases; i++) {
            MergePhase mergePhase = MergePhase.FACTORY.create();
            mergePhase.readFrom(in);
            phases.add(mergePhase);
        }
        this.mergePhases = phases;
    }
    ...
    public void writeTo(StreamOutput out) throws IOException {
        ...
        if (mergePhases == null) {
            ...
            out.writeVInt(mergePhases.size());
            for (MergePhase mergePhase : mergePhases) {
                mergePhase.writeTo(out);
            }
        }
    }
